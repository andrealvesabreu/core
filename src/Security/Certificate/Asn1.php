...
class Asn1
{
     ...
     */
    public static function getCNPJ(string $publickeyUnformated): ?string
    {
        // CNPJ OID = 2.16.76.1.3.3
        $cnpj = self::getOIDdata('2.16.76.1.3.3', $publickeyUnformated);
        return empty($cnpj) ? null : $cnpj;
    }

    /**
     * CPF OID = 2.16.76.1.3.1 e
     ...
     */
    public static function getCPF(string $publickeyUnformated): ?string
    {
        // CPF OID = 2.16.76.1.3.1
        $resp = self::getOIDdata('2.16.76.1.3.1', $publickeyUnformated);
        return empty($resp) ? null : substr($resp, 8, 11);
    }
     ...
     * @return string
     */
    public static function getOIDdata(string $oidNumber, string $publickeyUnformated): string
    {
        $ret = '';
     ...
     */
    protected static function oidHexMarker(string $oid): string
    {
        $abBinary = array();
        $partes = explode('.', $oid);
        $bun = 0;
     ...
     * @return array
     */
    protected static function xBase128(?array $abIn = null, int $qIn, bool $flag): array
    {
        $abc = $abIn;
        if ($qIn > 127) {
